...
G_DEFINE_FINAL_TYPE_WITH_CODE (BevyFullscreenBox, bevy_fullscreen_box, GTK_TYPE_WIDGET,
...
static void
show_ui (BevyFullscreenBox *self)
{
...
static void
hide_ui (BevyFullscreenBox *self)
{
  g_clear_handle_id (&self->timeout_id, g_source_remove);
...
static void
hide_timeout_cb (BevyFullscreenBox *self)
{
  self->timeout_id = 0;
...
static void
start_hide_timeout (BevyFullscreenBox *self)
{
...
static double
get_titlebar_area_height (BevyFullscreenBox *self)
{
...
static gboolean
is_focusing_titlebar (BevyFullscreenBox *self)
{
  GList *l;
...
static void
motion_cb (BevyFullscreenBox *self,
           double               x,
           double               y)
{
  self->is_touch = FALSE;
...
static void
enter_cb (BevyFullscreenBox *self,
          double               x,
          double               y)
{
  motion_cb (self, x, y);
...
static void
press_cb (BevyFullscreenBox *self,
          int                  n_press,
          double               x,
          double               y,
          GtkGesture          *gesture)
...
static void
set_focus (BevyFullscreenBox *self,
           GtkWidget           *widget)
{
...
static void
notify_focus_cb (BevyFullscreenBox *self,
                 GParamSpec          *pspec,
                 GtkRoot             *root)
{
...
static void
notify_reveal_cb (BevyFullscreenBox *self)
{
  g_object_notify_by_pspec (G_OBJECT (self), props[PROP_REVEALED]);
}

static void
bevy_fullscreen_box_root (GtkWidget *widget)
{
  BevyFullscreenBox *self = BEVY_FULLSCREEN_BOX (widget);
  GtkRoot *root;

  GTK_WIDGET_CLASS (bevy_fullscreen_box_parent_class)->root (widget);
...
static void
bevy_fullscreen_box_unroot (GtkWidget *widget)
{
  BevyFullscreenBox *self = BEVY_FULLSCREEN_BOX (widget);
  ...
  GTK_WIDGET_CLASS (bevy_fullscreen_box_parent_class)->unroot (widget);
}

static void
bevy_fullscreen_box_get_property (GObject    *object,
                                    guint       prop_id,
                                    GValue     *value,
                                    GParamSpec *pspec)
{
  BevyFullscreenBox *self = BEVY_FULLSCREEN_BOX (object);

  switch (prop_id) {
    case PROP_FULLSCREEN:
      g_value_set_boolean (value, bevy_fullscreen_box_get_fullscreen (self));
      break;

    case PROP_AUTOHIDE:
      g_value_set_boolean (value, bevy_fullscreen_box_get_autohide (self));
      break;

    case PROP_CONTENT:
      g_value_set_object (value, bevy_fullscreen_box_get_content (self));
      break;
...
static void
bevy_fullscreen_box_set_property (GObject      *object,
                                    guint         prop_id,
                                    const GValue *value,
                                    GParamSpec   *pspec)
{
  ...
  switch (prop_id) {
    case PROP_FULLSCREEN:
      bevy_fullscreen_box_set_fullscreen (self, g_value_get_boolean (value));
      break;

    case PROP_AUTOHIDE:
      bevy_fullscreen_box_set_autohide (self, g_value_get_boolean (value));
      break;

    case PROP_CONTENT:
      bevy_fullscreen_box_set_content (self, g_value_get_object (value));
      break;
...
static void
bevy_fullscreen_box_dispose (GObject *object)
{
  BevyFullscreenBox *self = BEVY_FULLSCREEN_BOX (object);
  ...
  G_OBJECT_CLASS (bevy_fullscreen_box_parent_class)->dispose (object);
}

static void
bevy_fullscreen_box_class_init (BevyFullscreenBoxClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

  object_class->get_property = bevy_fullscreen_box_get_property;
  object_class->set_property = bevy_fullscreen_box_set_property;
  object_class->dispose = bevy_fullscreen_box_dispose;

  widget_class->root = bevy_fullscreen_box_root;
  widget_class->unroot = bevy_fullscreen_box_unroot;
...
static void
bevy_fullscreen_box_init (BevyFullscreenBox *self)
{
  AdwToolbarView *toolbar_view;
...
BevyFullscreenBox *
bevy_fullscreen_box_new (void)
{
  return g_object_new (BEVY_TYPE_FULLSCREEN_BOX, NULL);
}

gboolean
bevy_fullscreen_box_get_fullscreen (BevyFullscreenBox *self)
{
  g_return_val_if_fail (BEVY_IS_FULLSCREEN_BOX (self), FALSE);

  return self->fullscreen;
}

void
bevy_fullscreen_box_set_fullscreen (BevyFullscreenBox *self,
                                      gboolean             fullscreen)
{
  g_return_if_fail (BEVY_IS_FULLSCREEN_BOX (self));
...
gboolean
bevy_fullscreen_box_get_autohide (BevyFullscreenBox *self)
{
  g_return_val_if_fail (BEVY_IS_FULLSCREEN_BOX (self), FALSE);
...
void
bevy_fullscreen_box_set_autohide (BevyFullscreenBox *self,
                                    gboolean             autohide)
{
  g_return_if_fail (BEVY_IS_FULLSCREEN_BOX (self));
...
GtkWidget *
bevy_fullscreen_box_get_content (BevyFullscreenBox *self)
{
  g_return_val_if_fail (BEVY_IS_FULLSCREEN_BOX (self), NULL);

  return adw_toolbar_view_get_content (self->toolbar_view);
}

void
bevy_fullscreen_box_set_content (BevyFullscreenBox *self,
                                   GtkWidget           *content)
{
  g_return_if_fail (BEVY_IS_FULLSCREEN_BOX (self));
  g_return_if_fail (content == NULL || GTK_IS_WIDGET (content));

  if (bevy_fullscreen_box_get_content (self) == content)
    return;
...
void
bevy_fullscreen_box_add_top_bar (BevyFullscreenBox *self,
                                   GtkWidget           *child)
{
  g_return_if_fail (BEVY_IS_FULLSCREEN_BOX (self));
...
void
bevy_fullscreen_box_add_bottom_bar (BevyFullscreenBox *self,
                                      GtkWidget           *child)
{
  g_return_if_fail (BEVY_IS_FULLSCREEN_BOX (self));
  g_return_if_fail (GTK_IS_WIDGET (child));
...
static void
bevy_fullscreen_box_add_child (GtkBuildable *buildable,
                                 GtkBuilder   *builder,
                                 GObject      *object,
                                 const char   *name)
{
  BevyFullscreenBox *self = (BevyFullscreenBox *)buildable;

  g_assert (BEVY_IS_FULLSCREEN_BOX (self));
  g_assert (GTK_IS_BUILDER (builder));
  g_assert (G_IS_OBJECT (object));

  if (g_strcmp0 (name, "top") == 0 && GTK_IS_WIDGET (object))
    bevy_fullscreen_box_add_top_bar (self, GTK_WIDGET (object));
  else if (g_strcmp0 (name, "bottom") == 0 && GTK_IS_WIDGET (object))
    bevy_fullscreen_box_add_bottom_bar (self, GTK_WIDGET (object));
  else
...
buildable_iface_init (GtkBuildableIface *iface)
{
  ...
  iface->add_child = bevy_fullscreen_box_add_child;
}

static void
clear_forced_timeout (gpointer data)
{
  BevyFullscreenBox *self = data;
...
void
bevy_fullscreen_box_reveal (BevyFullscreenBox *self)
{
  g_return_if_fail (BEVY_IS_FULLSCREEN_BOX (self));
...
void
bevy_fullscreen_box_unreveal (BevyFullscreenBox *self)
{
  g_return_if_fail (BEVY_IS_FULLSCREEN_BOX (self));
