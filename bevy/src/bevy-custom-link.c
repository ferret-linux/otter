...
G_DEFINE_FINAL_TYPE (BevyCustomLink, bevy_custom_link, G_TYPE_OBJECT)

static GParamSpec *properties[N_PROPS];

static void
bevy_custom_link_finalize (GObject *object)
{
  BevyCustomLink *self = BEVY_CUSTOM_LINK (object);

  g_clear_pointer (&self->pattern, g_free);
  g_clear_pointer (&self->target, g_free);
  g_clear_pointer (&self->compiled_regex, vte_regex_unref);

  G_OBJECT_CLASS (bevy_custom_link_parent_class)->finalize (object);
}

static void
bevy_custom_link_get_property (GObject    *object,
                                 guint       property_id,
                                 GValue     *value,
                                 GParamSpec *pspec)
{
  ...
  switch (property_id)
    {
    case PROP_PATTERN:
      g_value_take_string (value, bevy_custom_link_dup_pattern (self));
      break;

    case PROP_TARGET:
      g_value_take_string (value, bevy_custom_link_dup_target (self));
      break;
...
static void
bevy_custom_link_set_property (GObject      *object,
                                 guint         property_id,
                                 const GValue *value,
                                 GParamSpec   *pspec)
{
  ...
  switch (property_id)
    {
    case PROP_PATTERN:
      bevy_custom_link_set_pattern (self, g_value_get_string (value));
      break;

    case PROP_TARGET:
      bevy_custom_link_set_target (self, g_value_get_string (value));
      break;
...
static void
bevy_custom_link_init (BevyCustomLink *self)
{
...
static void
bevy_custom_link_class_init (BevyCustomLinkClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->finalize = bevy_custom_link_finalize;
  object_class->set_property = bevy_custom_link_set_property;
  object_class->get_property = bevy_custom_link_get_property;
...
BevyCustomLink *
bevy_custom_link_new (void)
{
  return g_object_new (BEVY_TYPE_CUSTOM_LINK, NULL);
}

BevyCustomLink *
bevy_custom_link_new_with_strings (const char *pattern,
                                     const char *target)
{
  return g_object_new (BEVY_TYPE_CUSTOM_LINK,
                       "pattern", pattern,
                       "target", target,
                       NULL);
}
...
char *
bevy_custom_link_dup_target (BevyCustomLink *self)
{
  g_return_val_if_fail (BEVY_IS_CUSTOM_LINK (self), NULL);

  return g_strdup (self->target);
}

void
bevy_custom_link_set_target (BevyCustomLink *self,
                               const char       *target)
{
  g_return_if_fail (BEVY_IS_CUSTOM_LINK (self));
...
char *
bevy_custom_link_dup_pattern (BevyCustomLink *self)
{
  g_return_val_if_fail (BEVY_IS_CUSTOM_LINK (self), NULL);
...
void
bevy_custom_link_set_pattern (BevyCustomLink *self,
                                const char       *pattern)
{
  g_return_if_fail (BEVY_IS_CUSTOM_LINK (self));
...
VteRegex *
bevy_custom_link_compile (BevyCustomLink *self)
{
...
char *
bevy_custom_link_substitute (BevyCustomLink *self,
                               const char       *subject)
{
  g_autoptr(GError) error = NULL;
