...
static void
bevy_close_dialog_confirm (AdwAlertDialog *dialog,
                             GArray           *requests,
                             gboolean          confirm)
{
  ...
  for (guint i = 0; i < requests->len; i++)
    {
      ...
      bevy_tab_force_quit (sr->tab);
    }
}

static void
bevy_close_dialog_response (AdwAlertDialog *dialog,
                              const char       *response,
                              GArray           *requests)
{
  GTask *task;
  ...
  if (!g_strcmp0 (response, "discard"))
    {
      bevy_close_dialog_confirm (dialog, requests, TRUE);
...
static AdwDialog *
_bevy_close_dialog_new (GtkWindow *parent,
                          GPtrArray *tabs)
{
  ...
  g_return_val_if_fail (BEVY_IS_TAB (g_ptr_array_index (tabs, 0)), NULL);

  if (tabs->len == 1)
    bevy_tab_raise (g_ptr_array_index (tabs, 0));
  ...
  for (guint i = 0; i < tabs->len; i++)
    {
      BevyTab *tab = g_ptr_array_index (tabs, i);
      ...
      if (bevy_tab_has_foreground_process (tab, &pid, &cmdline))
        {
      ...
      else
        {
          ...
          subtitle = bevy_tab_dup_subtitle (tab);
        }
  ...
  g_signal_connect_data (dialog,
                         "response",
                         G_CALLBACK (bevy_close_dialog_response),
...
void
_bevy_close_dialog_run_async (GtkWindow           *parent,
                                GPtrArray           *tabs,
                                ...
                                gpointer             user_data)
{
  ...
  dialog = _bevy_close_dialog_new (parent, tabs);
  task = g_task_new (dialog, cancellable, callback, user_data);
  g_task_set_source_tag (task, _bevy_close_dialog_run_async);
...
gboolean
_bevy_close_dialog_run_finish (GAsyncResult  *result,
                                 GError       **error)
{
