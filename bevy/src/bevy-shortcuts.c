...
enum {
  PROP_0,
  PROP_SETTINGS,
#define BEVY_SHORTCUT_DEFINE(NAME, name) PROP_##NAME,
# include "bevy-shortcuts.defs"
#undef BEVY_SHORTCUT_DEFINE
  N_PROPS
};

G_DEFINE_FINAL_TYPE (BevyShortcuts, bevy_shortcuts, G_TYPE_OBJECT)
...
static void
bevy_shortcuts_settings_changed_cb (BevyShortcuts *self,
                                      const char      *key,
                                      GSettings       *settings)
{
  g_assert (BEVY_IS_SHORTCUTS (self));
  g_assert (key != NULL);
...
static void
bevy_shortcuts_constructed (GObject *object)
{
  BevyShortcuts *self = (BevyShortcuts *)object;

  G_OBJECT_CLASS (bevy_shortcuts_parent_class)->constructed (object);
  ...
  g_signal_connect_object (self->settings,
                           "changed",
                           G_CALLBACK (bevy_shortcuts_settings_changed_cb),
                           self,
                           G_CONNECT_SWAPPED);
...
static void
bevy_shortcuts_dispose (GObject *object)
{
  BevyShortcuts *self = (BevyShortcuts *)object;

  g_clear_object (&self->settings);

  G_OBJECT_CLASS (bevy_shortcuts_parent_class)->dispose (object);
}

static void
bevy_shortcuts_get_property (GObject    *object,
                               guint       prop_id,
                               GValue     *value,
                               GParamSpec *pspec)
{
  BevyShortcuts *self = BEVY_SHORTCUTS (object);
...
#define BEVY_SHORTCUT_DEFINE(NAME, name) \
    case PROP_##NAME: \
...
static void
bevy_shortcuts_set_property (GObject      *object,
                               guint         prop_id,
                               const GValue *value,
                               GParamSpec   *pspec)
{
...
static void
bevy_shortcuts_class_init (BevyShortcutsClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->constructed = bevy_shortcuts_constructed;
  object_class->dispose = bevy_shortcuts_dispose;
  object_class->get_property = bevy_shortcuts_get_property;
  object_class->set_property = bevy_shortcuts_set_property;
...
static void
bevy_shortcuts_init (BevyShortcuts *self)
{
}

BevyShortcuts *
bevy_shortcuts_new (GSettings *settings)
{
  g_return_val_if_fail (!settings || G_IS_SETTINGS (settings), NULL);

  return g_object_new (BEVY_TYPE_SHORTCUTS,
                       "settings", settings,
                       NULL);
}
...
static GMenuItem *
bevy_shortcuts_copy_menu_item (GMenu *menu,
                                 guint  index)
{
  GMenuItem *copy;
...
static void
bevy_shortcuts_replace_key (GMenu      *menu,
                              guint       index,
                              const char *key,
                              const char *value)
{
  ...
  copy = bevy_shortcuts_copy_menu_item (menu, index);
...
/**
 * bevy_shortcuts_update_menu:
 * @self: a #BevyShortcuts
 * @menu: a #GMenu
 *
 ...
 */
void
bevy_shortcuts_update_menu (BevyShortcuts *self,
                              GMenu           *menu)
{
  GObjectClass *klass;
  guint n_items;

  g_return_if_fail (BEVY_IS_SHORTCUTS (self));
  ...
  for (guint i = 0; i < n_items; i++)
    {
      ...
      if ((pspec = g_object_class_find_property (klass, id)) &&
          G_IS_PARAM_SPEC_STRING (pspec))
        {
          ...
          bevy_shortcuts_replace_key (menu, i, "accel", accel);
        }
    }
}

/**
 * bevy_shortcuts_get_default_accelerator:
 ...
 */
char *
bevy_shortcuts_get_default_accelerator (const char *shortcut_name)
{
