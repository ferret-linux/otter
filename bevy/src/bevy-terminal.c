...
G_DEFINE_FINAL_TYPE (BevyTerminal, bevy_terminal, VTE_TYPE_TERMINAL)
...
static void
bevy_terminal_update_colors (BevyTerminal *self)
{
  const BevyPaletteFace *face;
  AdwStyleManager *style_manager;
  gboolean dark;

  g_assert (BEVY_IS_TERMINAL (self));

  style_manager = adw_style_manager_get_default ();
  dark = adw_style_manager_get_dark (style_manager);

  if (self->palette == NULL)
    self->palette = bevy_palette_lookup ("gnome");

  face = bevy_palette_get_face (self->palette, dark);
...
static void
bevy_terminal_toast (BevyTerminal *self,
                       int             timeout,
                       const char     *title)
{
...
static gboolean
bevy_terminal_is_active (BevyTerminal *self)
{
  BevyTerminal *active_terminal = NULL;
  BevyWindow *window;
  BevyTab *active_tab;
  ...
  if ((window = BEVY_WINDOW (gtk_widget_get_ancestor (GTK_WIDGET (self), BEVY_TYPE_WINDOW))) &&
      (active_tab = bevy_window_get_active_tab (window)))
    active_terminal = bevy_tab_get_terminal (active_tab);

  return active_terminal == self;
}

static void
bevy_terminal_update_clipboard_actions (BevyTerminal *self)
{
...
static void
bevy_terminal_update_url_actions (BevyTerminal *self,
                                    double          x,
                                    double          y)
{
...
static gboolean
bevy_terminal_match_clicked (BevyTerminal  *self,
                               double           x,
                               double           y,
                               int              button,
...
static void
bevy_terminal_setup_context_menu (VteTerminal           *terminal,
                                    const VteEventContext *context)
{
  BevyTerminal *self = (BevyTerminal *)terminal;
  double x, y;
  ...
  bevy_terminal_update_clipboard_actions (self);

  vte_event_context_get_coordinates (context, &x, &y);

  bevy_terminal_update_url_actions (self, x, y);
...
static void
bevy_terminal_capture_click_pressed_cb (BevyTerminal  *self,
                                          int              n_press,
                                          double           x,
                                          double           y,
                                          GtkGestureClick *click)
{
  ...
  if (n_press == 1 &&
    ...
    {
      if (hyperlink != NULL)
        handled = bevy_terminal_match_clicked (self, x, y, button, state, hyperlink);
      else if (match != NULL)
        {
          BevyCustomLink *custom_link = g_hash_table_lookup (self->custom_links, GINT_TO_POINTER (tag));

          if (custom_link != NULL)
            {
              g_autofree gchar *subst = bevy_custom_link_substitute (custom_link, match);

              if (subst != NULL)
                handled = bevy_terminal_match_clicked (self, x, y, button, state, subst);
            }
          else
            handled = bevy_terminal_match_clicked (self, x, y, button, state, match);
        }
    }
...
static void
bevy_terminal_scroll_to_bottom (BevyTerminal *self)
{
...
static gboolean
bevy_terminal_capture_key_pressed_cb (BevyTerminal     *self,
                                        guint               keyval,
                                        guint               keycode,
                                        GdkModifierType     state,
...
select_all_action (GtkWidget  *widget,
                   ...
                   GVariant   *param)
{
  g_assert (BEVY_IS_TERMINAL (widget));
...
copy_clipboard_action (GtkWidget  *widget,
                       ...
                       GVariant   *param)
{
  BevyTerminal *self = BEVY_TERMINAL (widget);
  ...
  if (text && text[0] != 0)
    {
      ...
      if (bevy_settings_get_toast_on_copy_clipboard (settings))
        bevy_terminal_toast (self, 1, _("Copied to clipboard"));
    }
}
                        ...
                        GVariant   *param)
{
  ...
  bevy_terminal_paste (BEVY_TERMINAL (widget));
}

static void
bevy_terminal_selection_changed (VteTerminal *terminal)
{
  bevy_terminal_update_clipboard_actions (BEVY_TERMINAL (terminal));
}
...
copy_link_address_action (GtkWidget  *widget,
                          ...
                          GVariant   *param)
{
  BevyTerminal *self = (BevyTerminal *)widget;
                  ...
                  GVariant   *param)
{
  ...
  if (bevy_str_empty0 (self->url))
    return;

  tab = BEVY_TAB (gtk_widget_get_ancestor (widget, BEVY_TYPE_TAB));

  bevy_tab_open_uri (tab, self->url);
}
...
static char *
bevy_terminal_get_portal_host_path (GFile *file)
{
...
static void
bevy_terminal_drop_file_list (BevyTerminal *self,
                                const GList    *files)
{
  ...
  for (const GList *iter = files; iter; iter = iter->next)
    {
      ...
      if (file_is_from_document_portal (file) &&
          (host_path = bevy_terminal_get_portal_host_path (file)))
        {
...
static void
bevy_terminal_drop_uri_list_line_cb (GObject      *object,
                                       GAsyncResult *result,
                                       gpointer      user_data)
{
  ...
  g_assert (state != NULL);
  g_assert (BEVY_IS_TERMINAL (state->terminal));
  ...
  if (line == NULL || g_strcmp0 (state->mime_type, TEXT_X_MOZ_URL) == 0)
    {
      bevy_terminal_drop_file_list (state->terminal, state->files);
  ...
  g_data_input_stream_read_line_async (line_reader,
                                       ...
                                       NULL,
                                       bevy_terminal_drop_uri_list_line_cb,
                                       g_steal_pointer (&state));
}

static void
bevy_terminal_drop_uri_list_cb (GObject      *object,
                                  GAsyncResult *result,
                                  gpointer      user_data)
{
  GdkDrop *drop = (GdkDrop *)object;
  g_autoptr(BevyTerminal) self = user_data;
  g_autoptr(GError) error = NULL;
...
static void
bevy_terminal_drop_file_list_cb (GObject      *object,
                                   GAsyncResult *result,
                                   gpointer      user_data)
{
  GdkDrop *drop = (GdkDrop *)object;
  ...
  if (!(value = gdk_drop_read_value_finish (drop, result, &error)))
    {
      ...
      if (g_error_matches (error, G_IO_ERROR, G_IO_ERROR_NOT_FOUND) ||
        ...
        gdk_drop_read_async (drop,
                             ...
                             DROP_REQUEST_PRIORITY,
                             NULL,
                             bevy_terminal_drop_uri_list_cb,
                             g_object_ref (self));
      else
        gdk_drop_finish (drop, 0);
  ...
  bevy_terminal_drop_file_list (self, file_list);
  gdk_drop_finish (drop, GDK_ACTION_COPY);
}

static void
bevy_terminal_drop_string_cb (GObject      *object,
                                GAsyncResult *result,
                                gpointer      user_data)
{
  GdkDrop *drop = (GdkDrop *)object;
...
static void
bevy_terminal_drop_moz_url_cb (GObject      *object,
                                 GAsyncResult *result,
                                 gpointer      user_data)
{
  GdkDrop *drop = (GdkDrop *)object;
...
static gboolean
bevy_terminal_drop_target_drop (BevyTerminal     *self,
                                  GdkDrop            *drop,
                                  double              x,
                                  double              y,
                                  GtkDropTargetAsync *drop_target)
{
  ...
  if (gdk_content_formats_contain_gtype (formats, GDK_TYPE_FILE_LIST) ||
    ...
    gdk_drop_read_value_async (drop,
                               ...
                               DROP_REQUEST_PRIORITY,
                               NULL,
                               bevy_terminal_drop_file_list_cb,
                               g_object_ref (self));
    return TRUE;
...
bevy_terminal_drop_target_drag_enter (BevyTerminal     *self,
                                        GdkDrop            *drop,
                                        double              x,
                                        double              y,
                                        GtkDropTargetAsync *drop_target)
...
static void
bevy_terminal_drop_target_drag_leave (BevyTerminal     *self,
                                        GdkDrop            *drop,
                                        GtkDropTargetAsync *drop_target)
{
  g_assert (BEVY_IS_TERMINAL (self));
...
static void
bevy_terminal_measure (GtkWidget      *widget,
                         ...
                         int            *natural_baseline)
{
  ...
  GTK_WIDGET_CLASS (bevy_terminal_parent_class)->measure (widget,
                                                            orientation,
                                                            for_size,
                                                            minimum,
                                                            natural,
...
static void
bevy_terminal_size_allocate (GtkWidget *widget,
                               int        width,
                               int        height,
                               int        baseline)
{
  ...
  GTK_WIDGET_CLASS (bevy_terminal_parent_class)->size_allocate (widget, width, height, baseline);
  ...
  if (bevy_terminal_is_active (self) &&
      GTK_IS_WINDOW (root) &&
...
/*
 * bevy_terminal_rewrite_snapshot:
 *
 ...
 *
 * By doing so, we allow our BevyTerminal widget to have padding
 ...
 */
static void
bevy_terminal_rewrite_snapshot (GtkWidget   *widget,
                                  GtkSnapshot *snapshot)
{
  ...
  GTK_WIDGET_CLASS (bevy_terminal_parent_class)->snapshot (widget, alternate);
...
static void
bevy_terminal_snapshot (GtkWidget   *widget,
                          GtkSnapshot *snapshot)
{
  BevyTerminal *self = BEVY_TERMINAL (widget);
  GtkBorder padding;
  ...
  bevy_terminal_rewrite_snapshot (widget, snapshot);
...
static void
bevy_terminal_shortcuts_notify_cb (BevyTerminal  *self,
                                     GParamSpec      *pspec,
                                     BevyShortcuts *shortcuts)
{
  g_assert (BEVY_IS_TERMINAL (self));
  ...
  bevy_shortcuts_update_menu (shortcuts, self->terminal_menu);
}

static void
bevy_terminal_emit_shell_precmd (BevyTerminal *self)
{
  g_assert (BEVY_IS_TERMINAL (self));
...
static void
bevy_terminal_emit_shell_preexec (BevyTerminal *self)
{
  g_assert (BEVY_IS_TERMINAL (self));
...
static void
notify_property_changed (BevyTerminal *self,
                         const char     *termprop,
                         GParamSpec     *property)
{
...
static void
bevy_terminal_char_size_changed (VteTerminal *terminal,
                                   guint        width,
                                   guint        height)
{
  BevyTerminal *self = (BevyTerminal *)terminal;
...
static void
bevy_terminal_constructed (GObject *object)
{
  BevyTerminal *self = (BevyTerminal *)object;

  g_assert (BEVY_IS_TERMINAL (self));

  G_OBJECT_CLASS (bevy_terminal_parent_class)->constructed (object);

  adw_style_manager_get_dark (adw_style_manager_get_default());
  g_signal_connect_object (adw_style_manager_get_default (),
                           "notify::color-scheme",
                           G_CALLBACK (bevy_terminal_update_colors),
                           self,
                           G_CONNECT_SWAPPED);
  g_signal_connect_object (adw_style_manager_get_default (),
                           "notify::dark",
...
static void
bevy_terminal_dispose (GObject *object)
{
  BevyTerminal *self = (BevyTerminal *)object;

  g_debug ("Disposing %s @ %p", G_OBJECT_TYPE_NAME (self), object);

  gtk_widget_dispose_template (GTK_WIDGET (self), BEVY_TYPE_TERMINAL);
  ...
  G_OBJECT_CLASS (bevy_terminal_parent_class)->dispose (object);
}

static void
bevy_terminal_get_property (GObject    *object,
                              guint       prop_id,
                              GValue     *value,
                              GParamSpec *pspec)
{
  BevyTerminal *self = BEVY_TERMINAL (object);
  ...
  switch (prop_id)
    ...
    case PROP_PALETTE:
      g_value_set_object (value, bevy_terminal_get_palette (self));
      break;
...
static void
bevy_terminal_set_property (GObject      *object,
                              guint         prop_id,
                              const GValue *value,
                              GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_PALETTE:
      bevy_terminal_set_palette (self, g_value_get_object (value));
      break;
...
static void
bevy_terminal_class_init (BevyTerminalClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);
  VteTerminalClass *terminal_class = VTE_TERMINAL_CLASS (klass);

  object_class->constructed = bevy_terminal_constructed;
  object_class->dispose = bevy_terminal_dispose;
  object_class->get_property = bevy_terminal_get_property;
  object_class->set_property = bevy_terminal_set_property;

  widget_class->measure = bevy_terminal_measure;
  widget_class->size_allocate = bevy_terminal_size_allocate;
  widget_class->snapshot = bevy_terminal_snapshot;

  terminal_class->selection_changed = bevy_terminal_selection_changed;
  terminal_class->setup_context_menu = bevy_terminal_setup_context_menu;
  terminal_class->char_size_changed = bevy_terminal_char_size_changed;
  ...
  gtk_widget_class_set_template_from_resource (widget_class, "/dev/itznoel/Bevy/bevy-terminal.ui");

  gtk_widget_class_bind_template_child (widget_class, BevyTerminal, drop_highlight);
  gtk_widget_class_bind_template_child (widget_class, BevyTerminal, drop_target);
  gtk_widget_class_bind_template_child (widget_class, BevyTerminal, popover);
  gtk_widget_class_bind_template_child (widget_class, BevyTerminal, size_label);
  gtk_widget_class_bind_template_child (widget_class, BevyTerminal, size_revealer);
  gtk_widget_class_bind_template_child (widget_class, BevyTerminal, terminal_menu);

  gtk_widget_class_bind_template_callback (widget_class, bevy_terminal_capture_click_pressed_cb);
  gtk_widget_class_bind_template_callback (widget_class, bevy_terminal_capture_key_pressed_cb);
  gtk_widget_class_bind_template_callback (widget_class, bevy_terminal_drop_target_drag_enter);
  gtk_widget_class_bind_template_callback (widget_class, bevy_terminal_drop_target_drag_leave);
  gtk_widget_class_bind_template_callback (widget_class, bevy_terminal_drop_target_drop);
...
static void
bevy_terminal_init (BevyTerminal *self)
{
  ...
  g_signal_connect_object (shortcuts,
                           "notify",
                           G_CALLBACK (bevy_terminal_shortcuts_notify_cb),
                           self,
                           G_CONNECT_SWAPPED);
  bevy_terminal_shortcuts_notify_cb (self, NULL, shortcuts);
  ...
  g_signal_connect (self,
                    ...
                    G_CALLBACK (bevy_terminal_emit_shell_precmd),
                    NULL);
  g_signal_connect (self,
                    "termprop-changed::" VTE_TERMPROP_SHELL_PREEXEC,
                    G_CALLBACK (bevy_terminal_emit_shell_preexec),
                    NULL);
  g_signal_connect (self,
  ...
  g_signal_connect_object (gtk_widget_get_clipboard (GTK_WIDGET (self)),
                           "changed",
                           G_CALLBACK (bevy_terminal_update_clipboard_actions),
                           self,
                           G_CONNECT_SWAPPED);
...
BevyPalette *
bevy_terminal_get_palette (BevyTerminal *self)
{
  g_return_val_if_fail (BEVY_IS_TERMINAL (self), NULL);

  return self->palette;
}

void
bevy_terminal_set_palette (BevyTerminal *self,
                             BevyPalette  *palette)
{
  g_return_if_fail (BEVY_IS_TERMINAL (self));

  if (g_set_object (&self->palette, palette))
    {
      bevy_terminal_update_colors (self);
      g_object_notify_by_pspec (G_OBJECT (self), properties[PROP_PALETTE]);
    }
}

const char *
bevy_terminal_get_current_container_name (BevyTerminal *self)
{
  gsize len;
...
const char *
bevy_terminal_get_current_container_runtime (BevyTerminal *self)
{
  gsize len;
...
static char *
bevy_terminal_dup_termprop_uri_by_id (BevyTerminal *self,
                                        guint           property)
{
...
char *
bevy_terminal_dup_current_directory_uri (BevyTerminal *self)
{
  g_return_val_if_fail (BEVY_IS_TERMINAL (self), NULL);

  return bevy_terminal_dup_termprop_uri_by_id (self, VTE_PROPERTY_ID_CURRENT_DIRECTORY_URI);
}

char *
bevy_terminal_dup_current_file_uri (BevyTerminal *self)
{
  g_autoptr(GUri) uri = NULL;

  g_return_val_if_fail (BEVY_IS_TERMINAL (self), NULL);

  return bevy_terminal_dup_termprop_uri_by_id (self, VTE_PROPERTY_ID_CURRENT_FILE_URI);
}

gboolean
bevy_terminal_can_paste (BevyTerminal *self)
{
  GdkClipboard *clipboard;

  g_return_val_if_fail (BEVY_IS_TERMINAL (self), FALSE);
...
void
bevy_terminal_paste (BevyTerminal *self)
{
  g_return_if_fail (BEVY_IS_TERMINAL (self));
  ...
  if (vte_terminal_get_scroll_on_keystroke (VTE_TERMINAL (self)))
    bevy_terminal_scroll_to_bottom (self);
}

void
bevy_terminal_reset_for_size (BevyTerminal *self)
{
  int width;
  int height;
  int cols;
...
void
bevy_terminal_update_custom_links_list (BevyTerminal *self,
                                          GListModel     *custom_links)
{
  ...
  for (guint i = 0; i < n_items; i++)
    {
      g_autoptr(BevyCustomLink) custom_link = g_list_model_get_item (custom_links, i);
      g_autoptr(VteRegex) regex = bevy_custom_link_compile (custom_link);
      int tag;
