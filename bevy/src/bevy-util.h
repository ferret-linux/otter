...
typedef enum
{
  BEVY_PROCESS_KIND_HOST    = 0,
  BEVY_PROCESS_KIND_FLATPAK = 1,
} BevyProcessKind;

BevyProcessKind   bevy_get_process_kind      (void) G_GNUC_CONST;
const char * const *bevy_host_environ          (void) G_GNUC_CONST;
char               *bevy_path_expand           (const char *path);
char               *bevy_path_collapse         (const char *path);
gboolean            bevy_shell_supports_dash_l (const char *shell);
gboolean            bevy_is_shell              (const char *arg0);
GListModel         *bevy_parse_shells          (const char *etc_shells);
const char         *bevy_app_name              (void) G_GNUC_CONST;
GVariant           *bevy_variant_new_toast     (const char *title,
                                                  guint       timeout);
gboolean            bevy_is_default            (void);
gboolean            bevy_make_default          (void);
char               *bevy_escape_underline      (const char *str);

static inline void
bevy_take_str (char **out_str,
                 char  *new_str)
{
  g_free (*out_str);
  *out_str = new_str;
