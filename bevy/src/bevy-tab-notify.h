...
static inline void
bevy_tab_notify_show_notification (BevyTabNotify *notify,
                                     const char      *cmdline)
{
  GtkRoot *window;

  g_assert (notify != NULL);
  g_assert (BEVY_IS_TAB (notify->tab));
  ...
  if (gtk_window_is_active (GTK_WINDOW (window)))
    {
      if (bevy_window_get_active_tab (BEVY_WINDOW (window)) == notify->tab)
        return;
    }
  else
    {
      ...
      const char *uuid = bevy_tab_get_uuid (notify->tab);
      ...
      g_notification_set_default_action_and_target (notification,
                                                    ...
                                                    "s", uuid);
      g_application_send_notification (G_APPLICATION (BEVY_APPLICATION_DEFAULT),
                                       uuid, notification);
    }

  bevy_tab_set_needs_attention (notify->tab, TRUE);
}

static inline void
bevy_tab_notify_shell_precmd_cb (BevyTerminal  *terminal,
  ...
  if (notify->current_cmdline != NULL)
    {
      ...
      if (elapsed_time >= TIME_WATERMARK_MSEC * 1000)
        bevy_tab_notify_show_notification (notify, notify->current_cmdline);
...
static void
bevy_tab_notify_shell_preexec_poll_cb (GObject      *object,
                                         GAsyncResult *result,
                                         gpointer      user_data)
{
  BevyTab *tab = (BevyTab *)object;
  BevyTabNotify *notify = user_data;

  g_assert (BEVY_IS_TAB (tab));
  g_assert (G_IS_ASYNC_RESULT (result));
  g_assert (notify != NULL);

  bevy_tab_poll_agent_finish (tab, result, NULL);

  g_set_str (&notify->current_cmdline,
             bevy_tab_get_command_line (tab));
}

static inline void
bevy_tab_notify_shell_preexec_cb (BevyTerminal  *terminal,
                                    BevyTabNotify *notify)
{
  g_assert (BEVY_IS_TERMINAL (terminal));
  g_assert (BEVY_IS_TAB (notify->tab));
   ...
   */
  bevy_tab_poll_agent_async (notify->tab,
                               NULL,
                               bevy_tab_notify_shell_preexec_poll_cb,
                               notify);
}

static inline void
bevy_tab_notify_init (BevyTabNotify *notify,
                        BevyTab       *tab)
{
  ...
  notify->shell_precmd_handler =
    g_signal_connect (terminal,
                      "shell-precmd",
                      G_CALLBACK (bevy_tab_notify_shell_precmd_cb),
                      notify);

  notify->shell_preexec_handler =
    g_signal_connect (terminal,
                      "shell-preexec",
                      G_CALLBACK (bevy_tab_notify_shell_preexec_cb),
                      notify);
}

static inline void
bevy_tab_notify_destroy (BevyTabNotify *notify)
{
  BevyTerminal *terminal = bevy_tab_get_terminal (notify->tab);
