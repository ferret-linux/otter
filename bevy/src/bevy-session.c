...
GVariant *
bevy_session_save (BevyApplication *app)
{
  BevySettings *settings;
  GVariantBuilder builder;
  gboolean restore_session;

  g_return_val_if_fail (BEVY_IS_APPLICATION (app), NULL);
  ...
  restore_session = bevy_settings_get_restore_session (settings);

  for (const GList *list = gtk_application_get_windows (GTK_APPLICATION (app));
       list != NULL;
       list = list->next)
    {
      if (BEVY_IS_WINDOW (list->data))
        {
          BevyWindow *window = BEVY_WINDOW (list->data);
          g_autoptr(GListModel) pages = bevy_window_list_pages (window);
              ...
              if (restore_session || pinned)
                {
                  ...
                  uuid = bevy_profile_get_uuid (profile);
                  default_container = bevy_profile_dup_default_container (profile);
                  container = bevy_tab_dup_container (tab);
                  is_active = bevy_window_get_active_tab (window) == tab;
                  ...
                  cwd = bevy_terminal_dup_current_directory_uri (terminal);
                   ...
                   */
                  if (cwd == NULL)
                    cwd = bevy_tab_dup_previous_working_directory_uri (tab);

                  G_GNUC_BEGIN_IGNORE_DEPRECATIONS
                  if (!(window_title = vte_terminal_get_window_title (VTE_TERMINAL (terminal))))
                    window_title = bevy_tab_get_initial_title (tab);
                  G_GNUC_END_IGNORE_DEPRECATIONS

                  if (container != NULL)
                    container_id = bevy_ipc_container_get_id (container);
...
gboolean
bevy_session_restore (BevyApplication *app,
                        GVariant          *state)
{
  ...
  g_return_val_if_fail (BEVY_IS_APPLICATION (app), FALSE);
  ...
  settings = bevy_application_get_settings (BEVY_APPLICATION_DEFAULT);
  restore_session = bevy_settings_get_restore_session (settings);
  restore_window_size = bevy_settings_get_restore_window_size (settings);
  ...
  while (g_variant_iter_loop (&iter, "@a{sv}", &window))
    {
      ...
      BevyWindow *the_window = NULL;
      BevyTab *active_tab = NULL;
      ...
      while (g_variant_iter_loop (&tab_iter, "@a{sv}", &tab))
        {
          g_autoptr(BevyIpcContainer) the_container = NULL;
          g_autoptr(BevyProfile) the_profile = NULL;
          BevyTerminal *terminal;
          const char *profile;
          ...
          if (!g_variant_lookup (tab, "zoom", "u", &zoom) || zoom >= BEVY_ZOOM_LEVEL_LAST)
            zoom = BEVY_ZOOM_LEVEL_DEFAULT;

          if (!bevy_str_empty0 (container))
            the_container = bevy_application_lookup_container (app, container);

          if (!bevy_str_empty0 (profile))
            the_profile = bevy_application_dup_profile (app, profile);

          if (the_profile == NULL)
            the_profile = bevy_application_dup_default_profile (app);

          if (the_window == NULL)
            the_window = bevy_window_new_empty ();

          the_tab = bevy_tab_new (the_profile);

          if (the_container != NULL)
            bevy_tab_set_container (the_tab, the_container);

          if (cwd != NULL)
            bevy_tab_set_previous_working_directory_uri (the_tab, cwd);

          if (window_title != NULL)
            bevy_tab_set_initial_title (the_tab, window_title);

          terminal = bevy_tab_get_terminal (the_tab);

          if (!maximized)
            vte_terminal_set_size (VTE_TERMINAL (terminal), columns, rows);

          if (zoom != BEVY_ZOOM_LEVEL_DEFAULT)
            bevy_tab_set_zoom (the_tab, zoom);

          bevy_window_add_tab_at_end (the_window, the_tab);
          bevy_window_set_tab_pinned (the_window, the_tab, pinned);
      ...
      if (the_window != NULL)
        {
          ...
          if (active_tab)
            {
              bevy_window_set_active_tab (the_window, active_tab);
