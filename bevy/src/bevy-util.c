...
static BevyProcessKind kind = BEVY_PROCESS_KIND_HOST;

G_DEFINE_CONSTRUCTOR(bevy_init_ctor)

BevyProcessKind
bevy_get_process_kind (void)
{
  return kind;
}

/**
 * bevy_shell_supports_dash_l:
 ...
 */
gboolean
bevy_shell_supports_dash_l (const char *shell)
{
  if (shell == NULL)
    return FALSE;
 ...
 */
char *
bevy_path_expand (const char *path)
{
  wordexp_t state = { 0 };
 ...
 */
char *
bevy_path_collapse (const char *path)
{
  ...
  expanded = bevy_path_expand (path);
...
bevy_init_ctor (void)
{
  if (g_file_test ("/.flatpak-info", G_FILE_TEST_EXISTS))
    kind = BEVY_PROCESS_KIND_FLATPAK;
}

gboolean
bevy_is_shell (const char *arg0)
{
...
GListModel *
bevy_parse_shells (const char *etc_shells)
{
  g_auto(GStrv) split = NULL;

  if (bevy_str_empty0 (etc_shells))
...
GVariant *
bevy_variant_new_toast (const char *title,
                          guint       timeout)
{
  GVariantBuilder b;
...
static void
bevy_make_default_in_file (const char *path)
{
...
gboolean
bevy_make_default (void)
{
  g_auto(GStrv) paths = get_xdg_terminals_list_paths ();

  for (guint i = 0; paths[i]; i++)
    bevy_make_default_in_file (paths[i]);

  return bevy_is_default ();
}

char *
bevy_escape_underline (const char *str)
{
  GString *gstr;
