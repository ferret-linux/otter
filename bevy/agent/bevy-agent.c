...
G_GNUC_UNUSED
static void
bevy_agent_quit (BevyAgent *agent,
                   int          exit_code)
{
...
G_GNUC_UNUSED
static gboolean
bevy_agent_init (BevyAgent  *agent,
                   int           socket_fd,
                   GError      **error)
{
  g_autoptr(BevySessionContainer) session = NULL;
  g_autoptr(BevyContainerProvider) podman = NULL;
  ...
  if (!(agent->bus = g_dbus_connection_new_sync (G_IO_STREAM (agent->stream),
                                                 ...
                                                 NULL,
                                                 error)) ||
      !(agent->impl = bevy_agent_impl_new (error)) ||
  ...
  session = bevy_session_container_new ();
  bevy_agent_impl_add_container (agent->impl, BEVY_IPC_CONTAINER (session));

  jhbuildrc = g_file_new_build_filename (g_get_home_dir (), ".config", "jhbuildrc", NULL);
  if (g_file_query_exists (jhbuildrc, NULL))
    {
      g_autoptr(BevySessionContainer) jhbuild_container = bevy_session_container_new ();
      bevy_ipc_container_set_id (BEVY_IPC_CONTAINER (jhbuild_container), "jhbuild");
      bevy_ipc_container_set_provider (BEVY_IPC_CONTAINER (jhbuild_container), "jhbuild");
      bevy_ipc_container_set_display_name (BEVY_IPC_CONTAINER (jhbuild_container), "JHBuild");
      bevy_ipc_container_set_icon_name (BEVY_IPC_CONTAINER (jhbuild_container), "container-jhbuild-symbolic");
      bevy_session_container_set_command_prefix (BEVY_SESSION_CONTAINER (jhbuild_container),
                                                   (const char * const []) { "jhbuild", "run", NULL });
      bevy_agent_impl_add_container (agent->impl, BEVY_IPC_CONTAINER (jhbuild_container));
    }

  podman = bevy_podman_provider_new ();
   ...
   */
  bevy_podman_provider_set_type_for_label (BEVY_PODMAN_PROVIDER (podman),
                                             "manager",
                                             "distrobox",
                                             BEVY_TYPE_DISTROBOX_CONTAINER);
  ...
  bevy_podman_provider_set_type_for_label (BEVY_PODMAN_PROVIDER (podman),
                                             ...
                                             NULL,
                                             BEVY_TYPE_TOOLBOX_CONTAINER);

  if (!bevy_podman_provider_update_sync (BEVY_PODMAN_PROVIDER (podman), NULL, &local_error))
    {
       ...
       */
      bevy_podman_provider_update_sync (BEVY_PODMAN_PROVIDER (podman), NULL, NULL);
    }

  bevy_agent_impl_add_provider (agent->impl, podman);
...
G_GNUC_UNUSED
static int
bevy_agent_run (BevyAgent *agent)
{
...
G_GNUC_UNUSED
static void
bevy_agent_destroy (BevyAgent *agent)
{
...
gint64
bevy_agent_get_default_rlimit_nofile (void)
{
      ...
      char *argv[])
{
  ...
  g_set_prgname ("bevy-agent");
  g_set_application_name ("bevy-agent");

  context = g_option_context_new ("- terminal container agent");
  g_option_context_add_main_entries (context, entries, GETTEXT_PACKAGE);
  if (!g_option_context_parse (context, &argc, &argv, &error) ||
      !bevy_agent_init (&agent, socket_fd, &error))
    {
  ...
  ret = bevy_agent_run (&agent);
  bevy_agent_destroy (&agent);
