...
static int
bevy_pty_create_producer (int      consumer_fd,
                            gboolean blocking)
{
  _g_autofd int ret = -1;
...
int
bevy_agent_pty_new_producer (int      consumer_fd,
                               GError **error)
{
  int fd;

  if (-1 == (fd = bevy_pty_create_producer (consumer_fd, FALSE)))
    {
      int errsv = errno;
...
int
bevy_agent_pty_new (GError **error)
{
...
void
bevy_agent_push_spawn (BevyRunContext   *run_context,
                         GUnixFDList        *fd_list,
                         const char         *cwd,
                         ...
                         GVariant           *env)
{
  ...
  g_return_if_fail (BEVY_IS_RUN_CONTEXT (run_context));
  g_return_if_fail (G_IS_UNIX_FD_LIST (fd_list));

  if (cwd == NULL || cwd[0] == 0)
    cwd = g_get_home_dir ();

  bevy_run_context_setenv (run_context, "PWD", cwd);
  bevy_run_context_set_cwd (run_context, cwd);

  bevy_run_context_append_args (run_context, argv);

  bevy_run_context_setenv (run_context, "COLORTERM", "truecolor");
  bevy_run_context_setenv (run_context, "TERM", "xterm-256color");
  bevy_run_context_setenv (run_context, "FLATPAK_TTY_PROGRESS", "1");
  ...
  if (env && g_variant_iter_init (&iter, env) > 0)
    {
      ...
      while (g_variant_iter_loop (&iter, "{&s&s}", &key, &value))
        bevy_run_context_setenv (run_context, key, value);
    }
      ...
      while (g_variant_iter_loop (&iter, "{uh}", &dest_fd_num, &handle))
        {
          ...
          if (-1 == (fd = g_unix_fd_list_get (fd_list, handle, &error)))
            {
              bevy_run_context_push_error (run_context, g_steal_pointer (&error));
              break;
            }

          bevy_run_context_take_fd (run_context, fd, dest_fd_num);
        }
    }
}

gboolean
bevy_agent_is_sandboxed (void)
{
