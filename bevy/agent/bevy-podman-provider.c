...
struct _BevyPodmanProvider
{
  BevyContainerProvider parent_instance;
...
G_DEFINE_TYPE (BevyPodmanProvider, bevy_podman_provider, BEVY_TYPE_CONTAINER_PROVIDER)

static void
bevy_podman_provider_storage_dir_changed_cb (BevyPodmanProvider *self,
                                               GFile                *file,
                                               GFile                *other_file,
                                               ...
                                               GFileMonitor         *monitor)
{
  ...
  g_assert (BEVY_IS_PODMAN_PROVIDER (self));
  ...
  if (g_strcmp0 (name, "db.sql") == 0)
    bevy_podman_provider_queue_update (self);
}

static void
bevy_podman_provider_constructed (GObject *object)
{
  BevyPodmanProvider *self = (BevyPodmanProvider *)object;
  g_autoptr(GFile) file = NULL;
  g_autoptr(GFile) storage_dir = NULL;
  g_autofree char *data_dir = NULL;
  g_autofree char *parent_dir = NULL;

  G_OBJECT_CLASS (bevy_podman_provider_parent_class)->constructed (object);
  ...
  if ((self->monitor = g_file_monitor (file, G_FILE_MONITOR_NONE, NULL, NULL)))
    g_signal_connect_object (self->monitor,
                             "changed",
                             G_CALLBACK (bevy_podman_provider_queue_update),
                             self,
                             G_CONNECT_SWAPPED);
  ...
  if ((self->storage_monitor = g_file_monitor_directory (storage_dir, G_FILE_MONITOR_NONE, NULL, NULL)))
    g_signal_connect_object (self->storage_monitor,
                             "changed",
                             G_CALLBACK (bevy_podman_provider_storage_dir_changed_cb),
                             self,
                             G_CONNECT_SWAPPED);
...
static void
bevy_podman_provider_dispose (GObject *object)
{
  BevyPodmanProvider *self = (BevyPodmanProvider *)object;
  ...
  G_OBJECT_CLASS (bevy_podman_provider_parent_class)->dispose (object);
}

static void
bevy_podman_provider_finalize (GObject *object)
{
  BevyPodmanProvider *self = (BevyPodmanProvider *)object;

  g_clear_pointer (&self->label_to_type, g_array_unref);

  G_OBJECT_CLASS (bevy_podman_provider_parent_class)->finalize (object);
}

static void
bevy_podman_provider_class_init (BevyPodmanProviderClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->constructed = bevy_podman_provider_constructed;
  object_class->dispose = bevy_podman_provider_dispose;
  object_class->finalize = bevy_podman_provider_finalize;
}

static void
bevy_podman_provider_init (BevyPodmanProvider *self)
{
  self->label_to_type = g_array_new (FALSE, FALSE, sizeof (LabelToType));
}

BevyContainerProvider *
bevy_podman_provider_new (void)
{
  return g_object_new (BEVY_TYPE_PODMAN_PROVIDER, NULL);
}

void
bevy_podman_provider_set_type_for_label (BevyPodmanProvider *self,
                                           const char           *key,
                                           const char           *value,
                                           GType                 container_type)
{
  ...
  g_return_if_fail (BEVY_IS_PODMAN_PROVIDER (self));
  g_return_if_fail (key != NULL);
  g_return_if_fail (g_type_is_a (container_type, BEVY_TYPE_PODMAN_CONTAINER));
...
static BevyPodmanContainer *
bevy_podman_provider_deserialize (BevyPodmanProvider *self,
                                    JsonObject           *object)
{
  g_autoptr(BevyPodmanContainer) container = NULL;
  ...
  gtype = BEVY_TYPE_PODMAN_CONTAINER;
  ...
  if (!bevy_podman_container_deserialize (container, object, &error))
    {
...
gboolean
_bevy_podman_provider_parse_json (BevyPodmanProvider  *self,
                                    const char            *json,
                                    GError               **error)
{
      ...
      for (guint i = 0; i < n_elements; i++)
        {
          ...
          if (JSON_NODE_HOLDS_OBJECT (element) &&
              ...
              (container = bevy_podman_provider_deserialize (self, element_object)))
            g_ptr_array_add (containers, g_steal_pointer (&container));
        }
    }

  bevy_container_provider_merge (BEVY_CONTAINER_PROVIDER (self), containers);

  return TRUE;
}

static void
bevy_podman_provider_communicate_cb (GObject      *object,
                                       GAsyncResult *result,
                                       gpointer      user_data)
{
  GSubprocess *subprocess = (GSubprocess *)object;
  BevyPodmanProvider *self;
  ...
  if (!_bevy_podman_provider_parse_json (self, stdout_buf, &error))
    {
...
static gboolean
bevy_podman_provider_update_source_func (gpointer user_data)
{
  BevyPodmanProvider *self = user_data;
  ...
  bevy_run_context_append_argv (run_context, "ps");
  bevy_run_context_append_argv (run_context, "--all");
  bevy_run_context_append_argv (run_context, "--format=json");

  subprocess = bevy_run_context_spawn_with_flags (run_context, G_SUBPROCESS_FLAGS_STDOUT_PIPE, &error);
  if (subprocess == NULL)
    return G_SOURCE_REMOVE;

  task = g_task_new (self, NULL, NULL, NULL);
  g_task_set_source_tag (task, bevy_podman_provider_update_source_func);
  ...
  g_subprocess_communicate_utf8_async (subprocess,
                                       NULL,
                                       NULL,
                                       bevy_podman_provider_communicate_cb,
...
void
bevy_podman_provider_queue_update (BevyPodmanProvider *self)
{
  g_return_if_fail (BEVY_IS_PODMAN_PROVIDER (self));
  ...
  if (self->queued_update == 0 && !self->is_updating)
    self->queued_update = g_timeout_add_seconds_full (G_PRIORITY_LOW,
                                                      ...
                                                      bevy_podman_provider_update_source_func,
                                                      self, NULL);
}

const char *
bevy_podman_provider_get_version (void)
{
  static char *version;
  ...
  if (version == NULL)
    {
      ...
      bevy_run_context_append_argv (run_context, "podman");
      bevy_run_context_append_argv (run_context, "version");
      bevy_run_context_append_argv (run_context, "--format=json");

      subprocess = bevy_run_context_spawn_with_flags (run_context, G_SUBPROCESS_FLAGS_STDOUT_PIPE, NULL);
      if (subprocess == NULL)
        return NULL;
...
gboolean
bevy_podman_provider_check_version (guint major,
                                      guint minor,
                                      guint micro)
{
  const char *version = bevy_podman_provider_get_version ();
...
gboolean
bevy_podman_provider_update_sync (BevyPodmanProvider  *self,
                                    ...
                                    GError               **error)
{
  ...
  g_return_val_if_fail (BEVY_IS_PODMAN_PROVIDER (self), FALSE);
