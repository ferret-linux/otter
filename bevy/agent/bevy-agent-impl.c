...
{
  BevyIpcAgentSkeleton parent_instance;
  GPtrArray *providers;
  GPtrArray *containers;
  guint has_listed_containers : 1;
};

static void agent_iface_init (BevyIpcAgentIface *iface);

G_DEFINE_TYPE_WITH_CODE (BevyAgentImpl, bevy_agent_impl, BEVY_IPC_TYPE_AGENT_SKELETON,
                         G_IMPLEMENT_INTERFACE (BEVY_IPC_TYPE_AGENT, agent_iface_init))
...
static void
bevy_agent_impl_finalize (GObject *object)
{
  BevyAgentImpl *self = (BevyAgentImpl *)object;

  g_clear_pointer (&self->containers, g_ptr_array_unref);
  g_clear_pointer (&self->providers, g_ptr_array_unref);

  G_OBJECT_CLASS (bevy_agent_impl_parent_class)->finalize (object);
}

static void
bevy_agent_impl_load_os_release (BevyAgentImpl *self)
{
  g_autofree char *contents = NULL;
  gsize len;

  g_assert (BEVY_IS_AGENT_IMPL (self));
          ...
          if ((end = strchr (begin, '"')))
            {
              ...
              bevy_ipc_agent_set_os_name (BEVY_IPC_AGENT (self), name);
            }
        }
    }
}
...
static void
bevy_agent_impl_constructed (GObject *object)
{
  BevyAgentImpl *self = (BevyAgentImpl *)object;

  G_OBJECT_CLASS (bevy_agent_impl_parent_class)->constructed (object);

  bevy_ipc_agent_set_user_data_dir (BEVY_IPC_AGENT (self), g_get_user_data_dir ());

  bevy_agent_impl_load_os_release (self);
}

static void
bevy_agent_impl_class_init (BevyAgentImplClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->constructed = bevy_agent_impl_constructed;
  object_class->finalize = bevy_agent_impl_finalize;
}

static void
bevy_agent_impl_init (BevyAgentImpl *self)
{
...
BevyAgentImpl *
bevy_agent_impl_get_default (void)
{
  static BevyAgentImpl *instance;

  if (g_once_init_enter (&instance))
    g_once_init_leave (&instance, g_object_new (BEVY_TYPE_AGENT_IMPL, NULL));

  return instance;
}

BevyAgentImpl *
bevy_agent_impl_new (GError **error)
{
  return g_object_ref (bevy_agent_impl_get_default ());
}

static void
bevy_agent_impl_provider_added_cb (BevyAgentImpl         *self,
                                     BevyIpcContainer      *container,
                                     BevyContainerProvider *provider)
{
  g_assert (BEVY_IS_AGENT_IMPL (self));
  g_assert (BEVY_IPC_IS_CONTAINER (container));
  g_assert (BEVY_IS_CONTAINER_PROVIDER (provider));

  bevy_agent_impl_add_container (self, container);
}

static void
bevy_agent_impl_provider_removed_cb (BevyAgentImpl         *self,
                                       BevyIpcContainer      *container,
                                       BevyContainerProvider *provider)
{
  ...
  for (guint i = 0; i < self->containers->len; i++)
    {
      BevyIpcContainer *element = g_ptr_array_index (self->containers, i);
      const char *element_id = bevy_ipc_container_get_id (element);
      ...
      if (g_strcmp0 (id, element_id) == 0)
        {
          ...
          if (self->has_listed_containers)
            bevy_ipc_agent_emit_containers_changed (BEVY_IPC_AGENT (self), i, 1, empty);
...
void
bevy_agent_impl_add_provider (BevyAgentImpl         *self,
                                BevyContainerProvider *provider)
{
  guint n_items;
  ...
  g_signal_connect_object (provider,
                           "added",
                           G_CALLBACK (bevy_agent_impl_provider_added_cb),
                           self,
                           G_CONNECT_SWAPPED);

  g_signal_connect_object (provider,
                           "removed",
                           G_CALLBACK (bevy_agent_impl_provider_removed_cb),
                           self,
                           G_CONNECT_SWAPPED);
  ...
  for (guint i = 0; i < n_items; i++)
    {
      g_autoptr(BevyIpcContainer) container = g_list_model_get_item (G_LIST_MODEL (provider), i);

      bevy_agent_impl_provider_added_cb (self, container, provider);
    }
}

void
bevy_agent_impl_add_container (BevyAgentImpl    *self,
                                 BevyIpcContainer *container)
{
  g_autofree char *object_path = NULL;
  g_autofree char *guid = NULL;

  g_return_if_fail (BEVY_IS_AGENT_IMPL (self));
  g_return_if_fail (BEVY_IPC_IS_CONTAINER (container));

  guid = g_dbus_generate_guid ();
  object_path = g_strdup_printf ("/dev/itznoel/Bevy/Containers/%s", guid);
  ...
  if (self->has_listed_containers)
    {
      ...
      bevy_ipc_agent_emit_containers_changed (BEVY_IPC_AGENT (self),
...
static gboolean
bevy_agent_impl_handle_list_containers (BevyIpcAgent        *agent,
                                          GDBusMethodInvocation *invocation)
{
  BevyAgentImpl *self = (BevyAgentImpl *)agent;
  ...
  bevy_ipc_agent_complete_list_containers (agent,
...
static gboolean
bevy_agent_impl_handle_create_pty (BevyIpcAgent        *agent,
                                     GDBusMethodInvocation *invocation,
...
static gboolean
bevy_agent_impl_handle_create_pty_producer (BevyIpcAgent        *agent,
                                              GDBusMethodInvocation *invocation,
                                              GUnixFDList           *in_fd_list,
                                              GVariant              *in_pty_fd)
{
  ...
  if (-1 == (producer_fd = bevy_agent_pty_new_producer (consumer_fd, &error)))
    goto return_gerror;
...
static void
bevy_agent_impl_get_preferred_shell_cb (GObject      *object,
                                          GAsyncResult *result,
                                          gpointer      user_data)
{
  ...
  agent = g_object_get_data (G_OBJECT (invocation), "BEVY_IPC_AGENT");
  ...
  bevy_ipc_agent_complete_get_preferred_shell (agent,
                                                 g_steal_pointer (&invocation),
                                                 default_shell);
}

static gboolean
bevy_agent_impl_handle_get_preferred_shell (BevyIpcAgent        *agent,
                                              GDBusMethodInvocation *invocation)
{
  const char *default_shell = "/bin/sh";
  struct passwd *pw;
  ...
  if (bevy_agent_is_sandboxed ())
    {
      g_autoptr(BevyRunContext) run_context = bevy_run_context_new ();
      ...
      bevy_run_context_append_argv (run_context, "sh");
      bevy_run_context_append_argv (run_context, "-c");
      bevy_run_context_append_argv (run_context, "/usr/bin/getent passwd $USER | cut -f 7 -d :");

      if ((subprocess = bevy_run_context_spawn_with_flags (run_context, G_SUBPROCESS_FLAGS_STDOUT_PIPE, NULL)))
        {
          ...
          g_subprocess_communicate_utf8_async (subprocess,
                                               NULL,
                                               NULL,
                                               bevy_agent_impl_get_preferred_shell_cb,
                                               g_steal_pointer (&invocation));
          return TRUE;
        }
    }
...
static gboolean
bevy_agent_impl_handle_discover_current_container (BevyIpcAgent        *agent,
                                                     GDBusMethodInvocation *invocation,
                                                     GUnixFDList           *in_fd_list,
                                                     GVariant              *in_pty_fd)
{
      ...
      if (g_strcmp0 (container_id, id) == 0)
        {
          ...
          bevy_ipc_agent_complete_discover_current_container (agent,
                                                                g_steal_pointer (&invocation),
                                                                NULL,
                                                                object_path);
          return TRUE;
...
static gboolean
bevy_agent_impl_handle_discover_proxy_environment (BevyIpcAgent        *agent,
                                                     GDBusMethodInvocation *invocation)
{
  ...
  bevy_ipc_agent_complete_discover_proxy_environment (agent,
                                                        g_steal_pointer (&invocation),
...
static void
agent_iface_init (BevyIpcAgentIface *iface)
{
  iface->handle_create_pty = bevy_agent_impl_handle_create_pty;
  iface->handle_create_pty_producer = bevy_agent_impl_handle_create_pty_producer;
  iface->handle_get_preferred_shell = bevy_agent_impl_handle_get_preferred_shell;
  iface->handle_list_containers = bevy_agent_impl_handle_list_containers;
  iface->handle_discover_current_container = bevy_agent_impl_handle_discover_current_container;
  iface->handle_discover_proxy_environment = bevy_agent_impl_handle_discover_proxy_environment;
}
