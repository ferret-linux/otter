...
{
  g_autoptr(BevyContainerProvider) provider = NULL;
  ...
  provider = bevy_podman_provider_new ();
  ...
  while ((name = g_dir_read_name (dir)))
    {
      ...
      r = _bevy_podman_provider_parse_json (BEVY_PODMAN_PROVIDER (provider), contents, &error);
      g_assert_no_error (error);
      g_assert_true (r);
    }
}
      ...
      char *argv[])
{
  ...
  g_test_add_func ("/Bevy/Podman/JSON", test_podman_json);
  return g_test_run ();
}
