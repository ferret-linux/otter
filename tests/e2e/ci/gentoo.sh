...
RUNTIME="${1:-podman}"
MODE="${2:-rootless}"
CONTAINER_NAME="otter-test-gentoo"
IMAGE="gentoo"
...
printf "==> gentoo | runtime=%s | mode=%s\n\n" "${RUNTIME}" "${MODE}"

run_step "registry pull" "${OTTER[@]}" reg pull "${ROOT_FLAG[@]}" "${IMAGE}"
run_step "create"        "${OTTER[@]}" create "${ROOT_FLAG[@]}" --image "${IMAGE}" --init --additional-packages "dev-vcs/git" --shell bash "${CONTAINER_NAME}"
