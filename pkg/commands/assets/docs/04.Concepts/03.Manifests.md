...
otter assemble to create or remove all (or selected)
...
| `home`       | string | Custom host directory to use as the container's home, same as `otter create --home`.                                  |
...
- **Scalar fields** (`image`, `clone`, `home`, `shell`, `hostname`,
  memory, etc.): the including entry's value wins if set; otherwise
  the base's value is used.
