...
- **Locking**: with `--all`, locked containers are silently skipped.
  When a locked container is named explicitly, it logs an error
  instead of removing it. Use `otter unlock` first, or pass
  `--bypass-lock`.
...
- [otter-generate-entry](otter-generate-entry.md) — desktop-entry
