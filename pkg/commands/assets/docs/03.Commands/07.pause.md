...
automatically skipped rather than treated as an error when using
`--all`, where only the subset of running containers is targeted.
Running `pause` against a non-running container by name instead reports
an error.
