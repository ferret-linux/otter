...
For a per-session prompt that uses the otter prompt instead of starship,
pass `--add-env OTTER_DISABLE_STARSHIP_INTEGRATION=1` (see
`How Otter Works` → "The default prompt"). To disable starship
persistently, create `/usr/lib/otter/container.no-starship` inside the
container instead.
