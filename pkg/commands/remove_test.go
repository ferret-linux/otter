...
// the per-app desktop entries created by otter-export, not the
// one created by `otter generate-entry`).
func writeExportedDesktopApp(t *testing.T, userHome, containerName string) string {
	...
	content := "[Desktop Entry]\nExec=/usr/bin/otter enter " + containerName + " -- some-app\n"
...
// `otter generate-entry <name>` so the generate-entry delete
		...
		ListContainersResult: []containermanager.Container{
			{
				Name:   containerName,
				Status: "Exited",
				Labels: map[string]string{"manager": "otter"},
			},
		},
