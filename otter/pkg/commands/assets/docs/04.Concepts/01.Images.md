...
Otter maintains a large curated registry of prebuilt, otter-optimized
...
| `nixos`               | `ghcr.io/ferret-linux/nixos-otr:stable`        | amd64, arm64  |
| `nixos-unstable`      | `ghcr.io/ferret-linux/nixos-otr:unstable`      | amd64, arm64  |
| `homebrew`            | `ghcr.io/ferret-linux/brew-otr:latest`         | amd64, arm64  |
| `guix`                | `ghcr.io/ferret-linux/guix-otr:latest`         | amd64, arm64  |
| `steamos`             | `ghcr.io/ferret-linux/steamos-otr:latest`      | amd64         |
| `amazonlinux`         | `ghcr.io/ferret-linux/amazon-otr:latest`       | amd64, arm64  |
| `artix`               | `ghcr.io/ferret-linux/artix-otr:openrc`        | amd64         |
| `slackware`           | `ghcr.io/ferret-linux/slackware-otr:stable`    | amd64         |
| `slackware-current`   | `ghcr.io/ferret-linux/slackware-otr:current`   | amd64         |

## Base images

Each entry above is built from an upstream base image via a
`Containerfile` under `images/` in the repository.
...
| `nixos`               | `nix.Containerfile`         | `ghcr.io/nixos/nix:latest`                         |
| `nixos-unstable`      | `nix.Containerfile`         | `ghcr.io/nixos/nix:latest`                         |
| `homebrew`            | `brew.Containerfile`        | `docker.io/library/debian:stable-slim`             |
| `guix`                | `guix.Containerfile`        | `docker.io/library/debian:stable-slim`             |
| `steamos`             | `steam.Containerfile`       | `docker.io/cachyos/cachyos-v3:latest`              |
| `amazonlinux`         | `amazon.Containerfile`      | `docker.io/library/amazonlinux:latest`             |
| `artix`               | `artix.Containerfile`       | `docker.io/artixlinux/artixlinux:base-openrc`      |
| `slackware`           | `slack.Containerfile`       | `docker.io/aclemons/slackware:latest`              |
| `slackware-current`   | `slack.Containerfile`       | `docker.io/aclemons/slackware:current`             |
...
- **A curated common package set.** Things like `bash`/`zsh`/`fish`/`nu`,
  ...
  are installed on top of the base image, plus `initsystem` where the
...
- **Image specific fixes.** Some images ship configurations that may
  ...
  already fix this at build time for a seamless user experience.
...
- `Registry` — how the registry catalog
...
- `otter create` — using an image to create
  a container.
- `Configuration` — setting a default image and
