...
def setup_generic(container_id):
    setup_pre_init(container_id)

    print("otter: Firing up init system...")
    print("Error: could not set up init system, no init found! Consider using an image that ships with an init system, or add it with \"--additional-packages\" during creation.!")
    sys.exit(1)


if __name__ == "__main__":
    container_id = sys.argv[1] if len(sys.argv) > 1 else ""
    setup_generic(container_id)