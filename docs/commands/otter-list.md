...
Lists every container that otter created and manages, along with its status
and image. By default only rootless containers are shown.
...
- Setting the `NO_COLOR` environment variable to any non-empty value disables
  colored/styled output.

## See Also

- [otter-inspect](otter-inspect.md) — detailed info for a single container.
- [otter-interactive](otter-interactive.md) — a TUI alternative to `list`.